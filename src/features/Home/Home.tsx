import { useState } from 'react';
import {
  Button, Card, CardProps, H1, Image, ListItem, ScrollView, Sheet, SizableText, Text, XStack,
  YStack
} from 'tamagui';

import { Bath, BedSingle, ChevronDown, ChevronUp, Scale3d } from '@tamagui/lucide-icons';
import { useIntl } from 'react-intl';
import { Dimensions, Image as Rimg } from 'react-native';
import { useLink } from 'solito/link';

import { useTranslation } from 'react-i18next';
import { Platform, StyleSheet } from 'react-native';
import { Link } from 'solito/link';
import buildings from '../../../buildings.json';
import { globalStyles } from '../../components/styles/GlobalStyle';

const dimensions = Dimensions.get('window');
const imageHeight = Math.round(dimensions.width * 9 / 16); //calculate with aspect ratio
const imageWidth = dimensions.width;
export function Home() {
  const { t, i18n } = useTranslation()
  const intl = useIntl()
  const linkProps = useLink({
    href: '/lists/hi',
  });
  return (
    <>
      <YStack jc="center" ai="center" p="$4" space>
        <YStack space="$4" maw={600}>
          <H1 ta="center" size={Platform.OS === 'web' ? '$9' : '$7'} fontFamily={'$mono'}>
            {intl.formatMessage({ id: 'Properties' })}
          </H1>
        </YStack>
      </YStack>
      <ScrollView bc="$background" px="$0" py="$6">
        <XStack
          flex={1}
          // $sm={{ flexDirection: 'row' }} 
          $xs={{
            px: '$0',
            py: '$0'
          }}
          $gtXs={{
            px: '$0'
          }}
          $sm={{
            px: '$2'
          }}
          $gtSm={{
            px: '$8'
          }}
          // space="$2"
          justifyContent='center'
          flexWrap='wrap'
        >
          {/* <DemoCard
          animation="bouncy"
          size="$5"
          w={250}
          h={300}
          scale={0.9}
          hoverStyle={{ scale: 0.925 }}
          pressStyle={{ scale: 0.875 }} /> */}
          {buildings?.hits?.length > 0 && buildings?.hits?.map((item: any, i: any) => {
            return (
              <YStack
                $xs={{
                  p: "$0"
                }}
                $gtXs={{
                  p: "$0"
                }}
                $gtSm={{
                  p: "$1"
                }}
              >
                <Link
                  key={item?.externalID}
                  href={`/lists/${item?.externalID}`}
                >
                  <DemoCard
                    key={item?.id}
                    animation="bouncy"
                    size="$5"
                    w={250}
                    h={350}
                    dataSet={item}
                    scale={0.9}
                    // hoverStyle={{ scale: 1.8 }}
                    overflow={'hidden'}
                  // pressStyle={{ scale: 2 }}
                  />
                </Link></YStack>)
          })}
        </XStack></ScrollView></>
  );
}
function DemoCard(props: CardProps) {
  const { dataSet }: any = props

  return (
    <Link
      // style={globalStyles.propertycardmain} 
      key={dataSet?.id} href={`/lists/${dataSet?.externalID}`}>
      <Card theme="light" bordered {...props} style={globalStyles.propertycardinner}>
        <Card.Header style={globalStyles.cardheader}>
          <Card.Background pos="relative">
            {Platform.OS === 'web' ? <Image
              pos="relative"
              width={300}
              height={200}
              resizeMode="cover"
              als="center"
              src={{ uri: dataSet?.coverPhoto?.url }}
            /> :
              <Rimg
                // src="data:image/jpeg;base64,"
                source={{ uri: dataSet?.coverPhoto?.url }}
                // style={{ height: 80, width: 80}}
                style={styles.backgroundImage}
                resizeMode={'cover'}
              />}
            <Text color={'green'} style={globalStyles.availablebadge} fontFamily={'$mono'}> Available </Text>
          </Card.Background>
        </Card.Header>
        <Card.Footer style={globalStyles.propertyCardBody}>
          <SizableText size="$4" color={Platform.OS === 'web' ? 'black' : 'black'}  >
            <Text numberOfLines={2} style={globalStyles.propertytitle} fontWeight={'$10'} fontFamily={'$mono'} textAlign={'left'} >{dataSet?.title}</Text>
            {/* <Label htmlFor="name">{dataSet?.title}</Label> */}
            <XStack style={globalStyles.propfeatures} als="center" flex={1} space="$2" boc="$color" br="$4" py="$2">
              <YStack>
                <ListItem alignItems='center' style={globalStyles.bedItem} px="$5" color={'black'} size="$2" fontWeight={'$10'} fontFamily={'$mono'} textAlign={'left'} icon={BedSingle} scaleIcon={1.4} scaleSpace={0.5}> {dataSet?.rooms}</ListItem>
              </YStack>
              <YStack>
                <ListItem alignItems='center' size="$2" color={'black'} fontWeight={'$10'} fontFamily={'$mono'} textAlign={'left'} icon={Bath} scaleIcon={1.4} scaleSpace={0.5}>{dataSet?.baths}</ListItem>
              </YStack>
              <YStack>
                <ListItem alignItems='center' size="$2" color={'black'} fontWeight={'$10'} fontFamily={'$mono'} textAlign={'left'} icon={Scale3d} scaleIcon={1.4} scaleSpace={0.5}>{dataSet?.area?.toFixed(0)}</ListItem>
              </YStack>
            </XStack>
            <YStack w="100%" maw={300}>
              <XStack h={20}>
                {/* <Separator als="stretch" vertical mx={15} /> */}
                <Text style={globalStyles.propertyprice} fontFamily={'$mono'} textAlign={'left'}>{dataSet?.price.toLocaleString('en-US', { style: 'currency', currency: 'AED', })}</Text>
              </XStack>
            </YStack>
          </SizableText>
        </Card.Footer>
      </Card>
    </Link>
  )
}
function SheetDemo() {
  const [open, setOpen] = useState(false);
  const [position, setPosition] = useState(0);
  return (
    <>
      <Button
        aria-label={'toggle-sheet-button'}
        size="$6"
        icon={open ? ChevronDown : ChevronUp}
        circular
        onPress={() => setOpen(x => !x)}
      />
      <Sheet
        modal
        open={open}
        onOpenChange={setOpen}
        snapPoints={[80]}
        position={position}
        onPositionChange={setPosition}
        dismissOnSnapToBottom>
        <Sheet.Overlay />
        <Sheet.Frame ai="center" jc="center">
          <Sheet.Handle />
          <H1 ta="center">What is Lorem Ipsum?</H1>

          <Button
            size="$6"
            circular
            icon={ChevronDown}
            aria-label={'close-sheet-button'}
            onPress={() => {
              setOpen(false);
            }}
          />
        </Sheet.Frame>
      </Sheet>
    </>
  );
}
const styles = StyleSheet.create({
  backgroundImage: {
    height: imageHeight,
    width: imageWidth,
    zIndex: 1,
  },
});